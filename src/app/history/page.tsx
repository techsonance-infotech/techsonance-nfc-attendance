  ...
  const loadAttendanceRecords = async () => {
    try {
      ...
      if (session?.user?.email) {
        const empResponse = await fetch(`/api/employees?email=${session.user.email}`);
        if (empResponse.ok) {
      ...
      // If employee (not admin), only show their records
      if (!isAdmin && currentEmployee) {
        url = `/api/attendance?employeeId=${currentEmployee.id}&limit=100`;
      }
      ...
      // Fetch employee details for each record (if admin viewing all)
      if (isAdmin) {
        ...
        // For employees, add their own info to records
        setRecords(data.map((record: AttendanceRecord) => ({
          ...record,
      ...
      <div className="max-w-lg mx-auto p-4 space-y-4">
        ...
        <div className="pt-4 pb-2">
          ...
          <p className="text-muted-foreground">
            {isAdmin ? "View all attendance records" : "View your attendance records"}
          </p>
        </div>
